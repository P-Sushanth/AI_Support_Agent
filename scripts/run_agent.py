...
def run_batch_inference(input_file: str, output_file: str, limit: int = None):
    ...
    print(f"Completed batch inference for {processed_count} tickets. Results saved to {output_file}")
    return results

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Baseline Support Agent batch inference")
    parser.add_argument("--input", type=str, default="data/processed/dev.jsonl", help="Input JSONL split")
    parser.add_argument("--output", type=str, default="results/baseline/batch_results.jsonl", help="Output JSONL results file")
    parser.add_argument("--limit", type=int, default=500, help="Max tickets to process")
    args = parser.parse_args()
    
    run_batch_inference(args.input, args.output, limit=args.limit)
