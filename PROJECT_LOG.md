...
## 🗃️ 2. Primary Dataset & Leakage Isolation
...
- **Deterministic Partitioning**:
  - `data/processed/train.jsonl` (**606 records / 60%**): Used exclusively to index the RAG knowledge base.
  - `data/processed/dev.jsonl` (**202 records / 20%**): Used for local prompt engineering and tuning.
  - `data/processed/golden_candidate.jsonl` (**203 records / 20%**): Isolated candidate split ensuring **0.0% data leakage** into RAG index.
...
| Agent Model | Intent Accuracy (95% Wilson CI) | Token Overlap F1 | Escalation Precision | Escalation Recall | Escalation F1 |
| :--- | :---: | :---: | :---: | :---: | :---: |
...
- **Human Spot-Check Set**: 30 hand-annotated tickets (`data/human_eval/human_spot_checks.jsonl`).
...
## 🔍 8. Systematic Failure Taxonomy (85 Identified Failures)

1. **`wrong_interpretation` (48 cases / Medium)**: Multi-symptom queries where agent misclassified primary intent.
2. **`retrieval_failure` (26 cases / Low)**: Relevant resolution context existed in train split but was missed by TF-IDF vectorizer.
3. **`unnecessary_escalation` (6 cases / Medium)**: Agent over-escalated routine self-service questions.
4. **`incorrect_escalation` (3 cases / HIGH)**: Agent missed mandatory security/safety escalation triggers.
5. **`incomplete_answer` (2 cases / Medium)**: Response missed required step (e.g. holding AirPods button 15s).
...
3. **Statistical Uncertainty**: At $n=200$, a $68.0\%$ accuracy carries a $\pm 6.4\text{ pp}$ margin of sampling error ($95\%\text{ CI: } [61.3\%, 74.1\%]$).
...
### Run Full Master Pipeline (Steps 1-7)
```bash
